...
%findall(X,(bio(X,_,_,_,_,_),descendant(X,henri4)),L),length(L,N),write(L).
%findall(X,(bio(X,_,_,_,_,_),descendant(louis17,X)),L),length(L,N),write(L).
%findall(X,(bio(X,_,_,_,_,_),descendant(henri4,X)),L),length(L,N),write(L).

:-dynamic(fact/2).
fact(1,1).

factDyn(N,X):-fact(N,X),!.
factDyn(N,X):-N1 is N-1,factDyn(N1,X1),X is N*X1,asserta(fact(N,X)).
